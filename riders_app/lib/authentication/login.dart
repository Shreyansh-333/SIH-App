...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future readDataAndSetDataLocally(User currentUser) async{
    await FirebaseFirestore.instance.collection("admins").doc(currentUser.uid).get().then((snapshot) async {
      if(snapshot.exists){
        if(snapshot.data()!["status"] == "approved"){
          await sharedPreferences!.setString("uid", currentUser.uid);
          await sharedPreferences!.setString("email", snapshot.data()!["adminEmail"]);
          await sharedPreferences!.setString("name", snapshot.data()!["adminName"]);
          await sharedPreferences!.setString("photoUrl", snapshot.data()!["adminAvatarUrl"]);
        ...
        children: [
          Container(
            ...
            child: Padding(
              ...
              child: Image.asset("images/signup.png", height: 270,),
            ),
          ),
          Form(
            key: _formKey,
